...
void solve(){
    ...
    while(!q.empty()){
        if(numberOfOrangesLeft == 0){
            break;
        }


        minutes++;
        int level = q.size(); //nos. of nodes in the current level
        ...
        for(int i=0;i<level;i++){
            ...
            for(int j=0;j<4;j++){
                pair<int,int> neigh = {node.first+dx[j],node.second+dy[j]};
                if(neigh.first >=0 && neigh.first<n && neigh.second>=0 && neigh.second<n && grid[neigh.first][neigh.second] == 1){
                    numberOfOrangesLeft--;
                    q.push(neigh);
